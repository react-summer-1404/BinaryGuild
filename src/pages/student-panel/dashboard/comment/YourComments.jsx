...
const YourComments = ({ profileData, item }) => {
  const { t } = useTranslation();
  const { author ,insertDate ,title ,disslikeCount ,likeCount ,describe} = item
  return (
      ...
      <div className="w-full">
        ...
        <div className="justify-start w-1/2 font-bold flex flex-nowrap gap-4">
          <div className="w-2/5 flex gap-2 rounded-4xl mt-2 flex-wrap">
            <Like/>
            <p className="m-auto mr-0">{likeCount}</p>
          </div>
          <div className="w-2/5 flex flex-wrap rounded-4xl mt-2 gap-2 ">
            <DisLike/>
            <p className="m-auto mr-0">{disslikeCount}</p>
          </div>
        </div>
      </div>
    </div>
