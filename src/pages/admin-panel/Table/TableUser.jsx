      ...
      <table className="min-w-full  divide-y divide-gray-600">
        <thead className="bg-gray-800 p-5 h-10 text-small text-gray-300 ">
          <tr className="">
            ...
            <th className=" text-start">{t("username")}</th>
            <th className=" text-start">{t("Roll")}</th>
            <th className=" text-start">{t("email")}</th>
            <th className=" w-35 text-start">{t("completeProfilePercent")}</th>
            <th className=" text-start">{t("situation")}</th>
            <th className="w-15">{t("function")}</th>
          </tr>
        </thead>
        <tbody>
          {users.listUser.map((user) => (
            <tr className=" text-[14px]" key={user.id}>
              <td className="pr-4 h-12 text-start">{user.fName}</td>
              <td className=" text-start">{user.userName}</td>
              <td className=" text-start">{user.roles}</td>
              <td className=" text-start">{user.gmail}</td>
              <td className=" text-start">
                {user.profileCompletionPercentage}
              </td>
              <td className=" text-start">
                {user.active ? t("active") : t("deActive")}
              </td>
