        ...
        <table className="min-w-full border border-gray-400 divide-y divide-gray-600">
          <thead className="bg-gray-800 p-5 h-10 text-small text-gray-300 ">
            <tr className="">
              ...
              <th className=" text-start">{t("reservedDate")}</th>
          ...
          <tbody>
            {usersDetails?.coursesReseves.map((item) => {
              item.courseId;
              return (
                <tr className="border border-gray-400 text-[14px] " key={item.courseId}>
                  <td className="pr-4 h-12 text-start">{item.courseName}</td>
                  <td className="text-start">{formatInsertDate}</td>
                  <td className="text-start">{item.accept ? t("isNotReserved") : t("isReserved") }</td>
