...
const UserManageFilter = () => {
  ...
  // const {data } = useQuery({
  //   queryKey:["user" , role],
  //   queryFn:()=>usersList(1,number,true,true,roleMap[role])
  // })
    ...
    <div className=" w-full border border-white bg-[#222] rounded-xl h-40">
      ...
      <div className="  h-23 w-full flex  items-center justify-around ">
        ...
        <div className=" w-1/3  h-20 flex flex-col">
          <p
            className="cursor-pointer rounded-[5px] bg-black-900 p-2"
          >{t("ChooseBasedOn")} {t("Roll")}
          </p>
          <select className="bg-black-600 mt-2 p-2 rounded-[5px] text-white cursor-pointer " value={role} onChange={(e)=>dispatch(setRole(e.target.value))}>
        ...
        <div className=" w-1/3  h-20 flex flex-col">
          <p
            className="rounded-[5px] bg-black-900 p-2"
          >{t("ChooseBasedOn")} {t("situation")}
          </p>
          <select className="bg-black-600 mt-2 p-2 rounded-[5px] text-white cursor-pointer " value={status} onChange={(e)=>dispatch(setStatus(e.target.value))}>
